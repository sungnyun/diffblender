...
class BasicTransformerBlock(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True, use_sp=True, use_nsp=True):
        super().__init__()
        ...
        if fuser_type == "gatedSA":
            ...
            if use_nsp:
                self.nsp_fuser = GatedSelfAttentionDense(key_dim, key_dim, n_heads, d_head) # match with text dim
        elif fuser_type == "gatedCA":
            if use_sp:
                self.sp_fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head) 
            if use_nsp:
                self.nsp_fuser = GatedCrossAttentionDense(key_dim, key_dim, value_dim, n_heads, d_head) # match with text dim
        elif fuser_type == "gatedSA-gatedCA":
            if use_sp:
                self.sp_fuser = GatedSelfAttentionDense(query_dim, key_dim, n_heads, d_head) 
            if use_nsp:
                self.nsp_fuser = GatedCrossAttentionDense(key_dim, key_dim, value_dim, n_heads, d_head) # match with text dim
        elif fuser_type == "gatedCA-gatedSA":
            if use_sp:
                self.sp_fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head) 
            if use_nsp:
                self.nsp_fuser = GatedSelfAttentionDense(key_dim, key_dim, n_heads, d_head) # match with text dim
    ...
    def forward(self, x, context, sp_objs, nsp_objs):
#        return checkpoint(self._forward, (x, context, objs), self.parameters(), self.use_checkpoint)
        if self.use_checkpoint and x.requires_grad:
            return checkpoint.checkpoint(self._forward, x, context, sp_objs, nsp_objs)
        else:
            return self._forward(x, context, sp_objs, nsp_objs)

    def _forward(self, x, context, sp_objs, nsp_objs): 
        x = self.attn1(self.norm1(x)) + x 
        if sp_objs is not None:
            x = self.sp_fuser(x, sp_objs) # identity mapping in the beginning 
        if nsp_objs is not None:
            context = self.nsp_fuser(context, nsp_objs) # identity mapping in the beginning
...
class SpatialTransformer(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True, use_sp=True, use_nsp=True):
        super().__init__()
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock(query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=use_checkpoint, use_sp=use_sp, use_nsp=use_nsp)
    ...
    def forward(self, x, context, sp_objs, nsp_objs):
        ...
        for block in self.transformer_blocks:
            x = block(x, context, sp_objs, nsp_objs)
