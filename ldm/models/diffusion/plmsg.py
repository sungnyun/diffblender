...
class PLMSGSampler(object):
    ...
    @torch.no_grad()
    def sample(self, S, shape, input, uc_dict=None, guidance_scale=1, guidance_mode_scale=[1], mask=None, x0=None):
        self.make_schedule(ddim_num_steps=S)
        return self.plms_sampling(shape, input, uc_dict, guidance_scale, guidance_mode_scale, mask=mask, x0=x0)


    @torch.no_grad()
    def plms_sampling(self, shape, input, uc_dict=None, guidance_scale=1, guidance_mode_scale=[1], mask=None, x0=None):
        ...
        for i, step in enumerate(time_range):
            print(f"PLMS sampling step : ({step}/{len(time_range)})")
            # set alpha 
            if self.alpha_generator_func != None:
            ...
            img, pred_x0, e_t = self.p_sample_plms(input, ts, index=index, uc_dict=uc_dict, guidance_scale=guidance_scale, guidance_mode_scale=guidance_mode_scale, old_eps=old_eps, t_next=ts_next)
            input["x"] = img
            old_eps.append(e_t)
            if len(old_eps) >= 4:
                old_eps.pop(0)
    ...
    @torch.no_grad()
    def p_sample_plms(self, input, t, index, guidance_scale=1., guidance_mode_scale=[1], uc_dict=None, old_eps=None, t_next=None):
        x = deepcopy(input["x"]) 
        b = x.shape[0]
        ...
        def get_model_output(input):
            ...
            if uc_dict is not None and guidance_scale != 1:
                unconditional_input = dict(
                     ...
                     context=uc_dict['context'], 
                     inpainting_extra_input=input["inpainting_extra_input"],
                     condition=uc_dict['condition'],
                     )
                unconditional_mode_input = dict(
                      x=input["x"], 
                     timesteps=input["timesteps"], 
                     context=input['context'], 
                     inpainting_extra_input=input["inpainting_extra_input"],
                     condition=uc_dict['mode_condition'],
                     )
                   
                e_t_uncond = self.model(unconditional_input)
                e_t_uncond_mode = self.model(unconditional_mode_input)
                mode_guidance = []
                for i, scale in enumerate(guidance_mode_scale):
                    mode_guidance.append(scale * (e_t[i] - e_t_uncond_mode[i]))
                mode_guidance = torch.stack(mode_guidance)
                e_t = e_t_uncond + guidance_scale * (e_t - e_t_uncond) + mode_guidance
            return e_t
