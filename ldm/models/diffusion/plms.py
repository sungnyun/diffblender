...
class PLMSSampler(object):
    ...
    def sample(self, S, shape, input, uc_dict=None, guidance_scale=1, mask=None, x0=None):
        ...
        return self.plms_sampling(shape, input, uc_dict, guidance_scale, mask=mask, x0=x0)


    @torch.no_grad()
    def plms_sampling(self, shape, input, uc_dict=None, guidance_scale=1, mask=None, x0=None):
        ...
        for i, step in enumerate(time_range):
            ...
            img, pred_x0, e_t = self.p_sample_plms(input, ts, index=index, uc_dict=uc_dict, guidance_scale=guidance_scale, old_eps=old_eps, t_next=ts_next)
            input["x"] = img
            old_eps.append(e_t)
            if len(old_eps) >= 4:
                old_eps.pop(0)
    ...
    @torch.no_grad()
    def p_sample_plms(self, input, t, index, guidance_scale=1., uc_dict=None, old_eps=None, t_next=None):
        x = deepcopy(input["x"]) 
        b = x.shape[0]
